...
The overview of the RAG pipeline on the diagram \ref{fig:rag-pipeline}.
...
prerequisites and their content are fetched for inclusion in the prompt. The purpose is to include prerequisites so that the LLM can explain them before addressing the user's question (RQ1).
...
The RAG pipeline described above is essential because the data varies significantly across courses. Fine-tuning the model is a big part of RAFT pipelines and makes it different from normal RAG.
...
The model fine-tuned for one epoch and 20 steps on Fine-tuning dataset \ref{fine_tune_dataset}.
