...
\[
\mathcal{L}_{\mathrm{RAFT}} = -\sum_{i} \log P_{\theta}(a_i \mid q_i, \mathcal{D}_i),
\]
...
where \(\mathbf{B} \in \mathbb{R}^{d \times r}\), \(\mathbf{A} \in \mathbb{R}^{r \times k}\), and the rank \(r \ll \min(d, k)\).\footnote{Here \(\min\) denotes the minimum function.} This formulation enables domain adaptation with minimal memory footprint and without catastrophic forgetting of the model's pre-trained knowledge.
